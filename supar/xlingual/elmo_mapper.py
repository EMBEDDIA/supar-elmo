...
class Elmogan():
    ...
    def map_batch(self, batch):
        for x,sentence in enumerate(batch):
            ...
            batch[x, 0, 0:seqlen, :] = apply_mapping(sentence[0], self.layer0)
            batch[x, 1, 0:seqlen, :] = apply_mapping(sentence[1], self.layer1)
            batch[x, 2, 0:seqlen, :] = apply_mapping(sentence[2], self.layer2)
        return batch
...
class Vecmap():
    ...
    def map_batch(self, batch):    
        for x,sentence in enumerate(batch):
            ...
            batch[x, 0, 0:seqlen, :] = apply_mapping(sentence[0], self.layer0)
            batch[x, 1, 0:seqlen, :] = apply_mapping(sentence[1], self.layer1)
            batch[x, 0:seqlen, :] = apply_mapping(sentence[2], self.layer2)
